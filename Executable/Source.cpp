...
int main(int argc, char *argv[])
{
    ...
    auto _kCluster = atoi(argv[2]);
    auto _mWeight = strtod(argv[3], nullptr);
    auto _Epsilon = strtod(argv[4], nullptr);
    ...
    _bResult = csv_write(_FileName, _Center, _Data, _DataCluster);
    if (false == _bResult)
    {
        return EXIT_FAILURE;
    }
...
bool csv_write(const _FileName& __filename, const vector<_Ty>& _Center, const vector<_Ty>& _Data, const vector<_Size>& _DataCluster)
{
    ...
    string _FileString(__filename);
    _FileString.erase(_FileString.find('.'));

    path _Directories(_FileString);
    _Directories /= to_string(_ClusterCount);
